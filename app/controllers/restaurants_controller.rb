  ...
  def vote
    ...
    if current_user && Time.now < current_round.date.to_datetime && voted_users <= users_without_admin && !params[:id].nil?
      ...
      if !user.nil? 
        ...
        userarchive = Userarchives.where(:user_id => user.id, :archives_id => current_round.id).first_or_create
        #if user.voted.nil? && params[:act].nil?
        if userarchive.voted_for.nil? && params[:act].nil?
          restaurant.increment(:votes, by = 1)
          #user.voted = params[:id]
          userarchive.voted_for = params[:id]
        #elsif !user.voted.nil? && !params[:act].nil?
        elsif !userarchive.voted_for.nil? && !params[:act].nil?
          restaurant.decrement(:votes, by = 1)
          #user.voted = nil
           userarchive.voted_for = nil
        end
        restaurant.save
        #user.save
        userarchive.save
